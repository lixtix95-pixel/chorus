...
class PixelValueLabel final : public juce::Label {
 void paint(juce::Graphics& g) override {
  ...
  if(!isBeingEdited()){g.setColour(juce::Colours::black);pixel::text(g,getText(),getLocalBounds(),14,true);}
 }
};
}
juce::Label* ChorusLookAndFeel::createSliderTextBox(juce::Slider&){
 ...
 l->setColour(juce::TextEditor::outlineColourId,juce::Colours::black);
 return l;
}
...
void ChorusLookAndFeel::drawButtonBackground(juce::Graphics& g,juce::Button& b,const juce::Colour&,bool hover,bool down){
 const bool filled=b.getToggleState()||down;g.fillAll(filled?juce::Colours::black:juce::Colours::white);
 g.setColour(juce::Colours::black);g.drawRect(b.getLocalBounds(),hover?3:2);
}
void ChorusLookAndFeel::drawButtonText(juce::Graphics& g,juce::TextButton& b,bool,bool down){
 g.setColour(b.getToggleState()||down?juce::Colours::white:juce::Colours::black);
...
MonomachineNovaAudioProcessorEditor::MonomachineNovaAudioProcessorEditor(MonomachineNovaAudioProcessor& p)
 ...
 setLookAndFeel(&look);
 look.setColour(juce::Slider::textBoxTextColourId,juce::Colours::black);
 look.setColour(juce::Slider::textBoxBackgroundColourId,juce::Colours::white);
 look.setColour(juce::Slider::textBoxOutlineColourId,juce::Colours::white);
 look.setColour(juce::Slider::textBoxHighlightColourId,juce::Colours::lightgrey);
 for(int i=0;i<8;++i){
...
void MonomachineNovaAudioProcessorEditor::paint(juce::Graphics& g){
 ...
 g.fillRect(18,83,684,33);g.setColour(juce::Colours::white);
